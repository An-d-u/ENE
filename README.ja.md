...
生活記録は、アプリが起動していなかった間に ENE がテキストベースの生活環境で何をしていたかを記録します。この機能はデフォルトで無効です。`設定 → 動作 → 生活記録` で有効にでき、最小非アクティブ時間のデフォルト値は 60 分です。同じ場所で変更できます。

自由形式の Markdown 生活環境は `設定 → プロンプト → 生活環境` で編集します。ランタイムファイルは `%AppData%/ENE/prompts/life_world.md` です。生活環境を意図的に空にすると生成をスキップし、通常チャットはそのまま続行します。

条件を満たす非アクティブ期間後の最初の通常チャットでは、LLM を順番に 2 回呼び出すことがあります。最初に生活記録を生成し、次に最新の成功記録を一時コンテキストへ含めて通常応答を生成します。そのターンに表示されるトークン使用量は両方の呼び出しを合算します。コマンドは最初のチャット条件を消費せず、応答をリロールしても生活記録は再生成されません。

`··· → 生活記録` から日付別に記録を参照できます。過去の記録は読み取り専用で、再生成できるのは最新の記録だけです。再生成に失敗した場合は以前の記録を保持します。記録ファイルの破損や生成失敗があっても、通常チャットの応答は続行します。

ENE のデータを移動または復元する前に、次の 3 つのユーザーファイルをまとめてバックアップしてください。

- `%AppData%/ENE/life_records.json`
- `%AppData%/ENE/life_session_state.json`
- `%AppData%/ENE/prompts/life_world.md`

保存、復旧、プライバシー、リリース確認については [生活記録の運用と復旧](docs/life_records.md) を参照してください。
