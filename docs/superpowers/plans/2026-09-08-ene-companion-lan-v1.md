...
> **2026-09-17 진행 갱신:** 사용자 승인으로 단말 설치 시험을 보류하고 [미디어 계획](2026-09-16-ene-companion-media.md)을 순차 실행했다. 그 계획의 A1~A4에서 이 문서 Task 7~10에 해당하는 실제 ENE 접수·공개 대화·트레이/종료 연결을 구현·자동 검증했으며, B~D에서 자동 음성·Live2D·쓰다듬기·공통 설정을 추가했다. 아래 과거 체크박스를 일괄 완료로 바꾸지 않고 실제 실행 항목과 차이는 미디어 계획에 기록한다. 최신 증거는 [미디어 검증 기록](../../companion-media-validation.md)을 따른다. Task 6의 실기기 LAN 인수와 후반의 단말/서명/출시 요구는 이 자동 검증으로 대체하지 않으며 전체 V1 출시 완료로 표시하지 않는다.
...
- [x] 실제 loopback 포트 0을 사용하는 테스트를 작성한다. production 포트는 8765이며 테스트의 port 0을 사용자 설정으로 노출하지 않는다. `/info` 최소 정보, 무인증/Origin 거절, QR 승낙 전 데이터 차단, header 인증, hello timeout, heartbeat·rate/size 제한·느린 수신자를 검증한다. 테스트 coroutine은 `asyncio.run`으로 실행해 새 pytest 플러그인을 추가하지 않는다.
- [x] `python -m pytest tests/test_companion_gateway.py tests/test_companion_session.py tests/test_companion_adapter.py tests/test_companion_dialog.py -q`를 실행해 실패를 확인한다.
- [x] adapter의 Qt `queued` signal에 불변 요청과 서버가 만든 correlation ID를 전달하고, 결과는 `loop.call_soon_threadsafe`로 돌려준다. GUI 스레드에서 Future 결과를 기다리지 않는다. controller는 Qt에서 서버 세대의 유효성을 확정한다. 승인 교체는 Qt 구등록 신규 수락 일시 차단→서버 저장 성공→Qt 등록 세대 확정→구연결 폐기→새 토큰 전송의 순서다. 저장 실패 시 파일이 이전 상태 그대로임을 확인한 경우에만 이전 등록을 다시 활성화한다. `storage_uncertain`이면 Qt 접수 차단을 유지하고 모든 모바일 연결을 종료하며 서버 재시작·등록 상태 재확인을 요구한다.
- [x] session은 수신 작업·한 개의 순서 있는 writer·heartbeat timer를 분리한다. snapshot을 부분 단위로 보내 제어 메시지를 처리할 기회를 보장하고, 수정 가능한 Qt 목록을 직접 순회하지 않는다. 종료는 Qt 차단 확인 후 서버 task cancel/await→소켓/runner 종료→스레드 정리다. 종료 중 controller를 먼저 파괴하지 않는다.
- [x] §4.1의 스냅샷 한 개 제한을 구현한다. 중복 sync는 병합하고 무효화 요청은 최신 한 건만 보관한다. 취소된 Qt 캡처/직렬화 결과를 세대로 거르고 이전 버퍼·구독을 해제한 뒤 교체한다. 큰 스냅샷을 보내는 동안 반복 sync·편집·reset을 주입해 동시 캡처/직렬화/송신 작업이 각각 한 개를 넘지 않는지, 해제되지 않은 버퍼가 요청 수에 비례해 쌓이지 않는지, 변경이 멈춘 뒤 최종 대화로 수렴하는지 검증한다.
- [x] 같은 등록의 새 일반 소켓은 토큰과 hello 검증을 모두 통과한 뒤 현재 소켓을 교체한다. 서버가 만든 연결 세대를 바꾸고 구소켓의 신규 명령·늦은 callback·heartbeat·writer·구독을 무효화한다. Qt에 아직 수락되지 않은 구연결 요청도 재검증해 거절하지만 이미 예약/수락한 작업과 원장은 유지하고 결과는 현재 연결에서 복구한다. 구소켓 정리를 완료하기 전 교체 요청은 제한하여 종료 대기 소켓이 누적되지 않게 한다. 두 연결 경합·잘못된 토큰·hello timeout·구소켓의 늦은 종료 이벤트로 정상 연결이 끊기지 않는 테스트를 추가한다.
- [x] 개발 실행 `python -m tools.companion_smoke`를 만든다. `TemporaryDirectory`와 가상 어댑터만 생성하고 실제 Settings/기억/LLM을 초기화하지 않는다. QDialog에 활성화·주소·QR·승인·거절·등록 해제·합성 응답 계수기를 제공한다. 자동 승인은 만들지 않는다. 위 테스트와 포트 충돌/시작 실패를 통과시킨 후 `feat: 동반 앱 게이트웨이와 연결 검증 화면 추가`로 커밋한다.

실행 기록: 미구현 실패부터 순차 구현한 뒤 Task 4 집중 시험 **48개**, 기존 계약·등록·원자 보관까지 합쳐 **185개 통과**. 실제 루프백에서 Qt 로컬 승인→새 토큰 인증→전체 동기화→합성 응답→동일 요청 재전송을 연결했고 응답 계수는 한 번만 증가했다. 5,000개 대화의 분할 전송·해시·허용 필드, 잘못된 인증/명령/크기, 생존 확인과 과속 제어, 느린 수신 큐 상한, 구연결 교체와 스레드 종료를 확인했다. 반복 무효화는 캡처/직렬화/송신을 겹쳐 만들지 않고 최신 상태로 수렴한다. 등록 실패 시 승인 소켓 종료 및 기존 연결 재동기화, 상태 갱신과 겹친 QR 만료의 누락을 실패 테스트로 재현·보완했다. 실제 ENE·Android·LAN 단말 시험 완료를 뜻하지 않는다.

비동기 강화 검사: `PYTHONASYNCIODEBUG=1`과 `-W error`에서 설치된 aiohttp 3.14.1 내부 `Request._transport_sockname`의 DeprecationWarning이 HTTP 생성을 중단하는 것을 분리 확인했다. 시험 명령에서만 `-W 'ignore:Setting custom Request._transport_sockname attribute is discouraged:DeprecationWarning:aiohttp.web_request'` 한 종류를 제외해 **48개 통과**했다. 다른 자원 경고는 제외하지 않았으며 발견된 시험 클라이언트의 WS 응답은 명시 종료하도록 수정했다. 기본 Ruff 검사도 통과했다. 헤드리스 Qt 폰트 DB가 비어 발생한 네모 글자는 렌더 시험에만 시스템 한글 폰트를 등록해 구분했고 QR·문구·승인 버튼 배치를 이미지로 확인했다. 이미지·실제 주소·토큰은 커밋하지 않는다. PC 전체 앱 최종 종료 경로와 실제 휴대폰 성능은 후속 체크포인트다.
...
2026-09-16 결정에 따른 TTS·Live2D·쓰다듬기·공통 설정은 확장 설계와 [별도 구현 계획](2026-09-16-ene-companion-media.md)에서 다룬다. 이 텍스트 계획의 선행 작업과 계약 보존 요구는 유지한다. 설치 시험은 보류되었으며 기분 전용 화면·화면 스트리밍·Tailscale은 여전히 추가하지 않는다.
