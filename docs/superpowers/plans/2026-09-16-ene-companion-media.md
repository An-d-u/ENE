...
**인계 전 보완(2026-09-17):** 문서와 보관 정책을 대조하다 Android 등록 변경 시 캐시 삭제 연결과 현재 캐릭터 상태 비보관이 빠져 있음을 확인했다. 디스크의 설정·장식 보정·표정 선택·동작 번호를 정적 기본값으로 제거하고 구형 상태 포함 캐시는 다음 초기화에서 폐기한다. 등록 해제·승인된 교체·인증된 등록 폐기 전에 연결/다운로드/렌더러 핀과 스트림을 회수하고 캐시를 삭제하며, 실패하면 기존 로컬 등록을 보존한다. 소유한 캐시 경로만 정리하고 알 수 없는 파일은 유지한다. 실패 재현 후 합성 시험 8개를 추가했으며 Android 전체 **44개 클래스·254개 시험 통과**, 실패·오류·제외 0; offline strict unit/lint/debug/instrumentation 빌드 통과(52초), Lint 오류 0·기존 경고 27개. 새 APK의 557항목·캐릭터 21항목·허용 파일 20개 해시 일치·금지 경로 후보 0을 재확인했다. APP 코드 커밋은 `03c1a56`; PC 소스는 E1의 `a66b9239` 이후 변경하지 않았다. APK 해시는 인계 문서에 기록한다.
...
- [x] 아래 인수표에 실제 수행 결과·명령·각 저장소 커밋·APK SHA-256을 적는다. 이전 성공 수를 복사하거나 실행하지 않은 테스트를 통과로 쓰지 않는다.
- [x] 자동 전환 조건, 시작 후 대체 재생 금지, 브라우저/기타 형식의 PC 전용 예외, PCM/WAV와 모델 상한, 권리 제한, 재등록 캐시 삭제, 오류 복구를 사용자 관점에서 설명한다.
- [x] 단말 미실시 항목을 유지한다. 서명·배포·설치 허가를 이 계획으로 추정하지 않는다. 기존 전체 V1 인수는 별도이며 이 계획의 자동 시험 완료로 대신 체크하지 않는다.
- [x] 문서 링크·UTF-8 BOM·diff 검사·개인정보 후보 확인 후 양쪽 `docs: 캐릭터 음성 지원과 검증 경계 기록`으로 커밋한다.

**E2 검증 기록(2026-09-17):** PC 검증 기록·LAN 수용표·기존 텍스트 계획과 APP 지원/기존 연결/설치 안내를 갱신했다. 구현·자동 검증 결과와 미실시 단말 인수를 분리하고, 과거 시험 수·APK 해시는 역사적 기록으로 명시했다. 변경 문서 7개 UTF-8/BOM 및 로컬 링크 19개, diff 공백 검사를 통과했다. 개인정보 후보는 정책·일반 기술 설명뿐이고 실제 대화·키·개인 경로는 추가하지 않았다. 최종 APK SHA-256은 `71c626aa665fb757f2d072c5b62bf391d28b662aee58d9840a095a1e742639ed`다. 원본 PC main과 두 작업 브랜치를 보존하고 병합·게시·서명·설치를 수행하지 않는다.
...
승인된 A0~E2를 순차 실행했다. 현재 상태는 **소스 구현 및 자동 검증 완료, 단말 인수 미실시**다. 실제 단말·서명·출시 경계는 별도 승인과 검증 없이 완료로 바꾸지 않는다.

**로컬 병합 후속 기록(2026-09-17):** E2 인계 뒤 사용자가 로컬 병합을 선택하여 PC `802f2874`, APP `dff9f44`를 각각 `main`에 fast-forward 병합했다. 병합 후 PC 전체 3976개 통과·1개 제외(83.91초), Ruff·캐릭터 계약 검사 통과; APP 단위 254개 재실행 통과(51초), offline strict Lint/개발·계측 APK 빌드 통과(15초), Lint 오류 0·기존 경고 27개. 새 개발 APK 해시는 `8aea6d200aa38d86f2dd6917134731de84b5ae724e287530a7a078de418d74a7`이며 이전 E2 시점 해시와 구분한다. 격리 작업 폴더의 비추적 실행 기록은 보존한다. 원격 게시·서명·설치·실기기 시험은 하지 않았다.
