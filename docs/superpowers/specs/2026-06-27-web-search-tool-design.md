...
검색 흐름 중 채팅창의 로딩 문구는 현재 작업 단계를 반영한다. 메시지 전송 직후와 최종 답변 생성 중에는 `생각중...`을 표시하고, Tavily 같은 외부 검색 API를 실제로 호출하는 동안에는 `검색중...`을 표시한다. 검색이 끝나면 최종 답변 생성 단계로 돌아가므로 다시 `생각중...`으로 복구한다.
...
src/core/bridge_workers.py
  AIWorker progress_changed

src/core/bridge.py
  request_pending_stage_changed
```
...
검색 진행 상태는 기존 `request_pending_changed(bool)`와 분리한다. pending 신호는 요청 중인지 여부만 유지하고, 새 단계 신호는 `thinking` 또는 `searching`만 전달한다. 프런트는 pending row를 그대로 재사용하되 문구만 단계에 맞게 교체한다.
...
3. Bridge가 pending 상태를 켜고 요청 단계를 `thinking`으로 알린다.
4. LLM 클라이언트가 메모리 컨텍스트를 만든다.
5. 검색 기능이 꺼져 있으면 기존 흐름을 그대로 사용한다.
6. 검색 기능이 켜져 있고 자동 검색이 켜져 있으면 검색 필요 여부를 판단한다.
7. 검색이 필요하면 요청 단계를 `searching`으로 바꾸고 `SearchTool.search()`를 호출한다.
8. 검색이 끝나면 요청 단계를 다시 `thinking`으로 바꾼다.
9. 검색 결과를 `[WEB_SEARCH_RESULTS]` 블록으로 변환한다.
10. 메모리 컨텍스트, 검색 컨텍스트, 사용자 메시지를 합쳐 모델에 전달한다.
11. 모델은 검색 결과에 포함된 URL만 출처로 사용해 답변한다.

수동 `/search` 명령은 6번의 자동 판단을 건너뛰고 바로 7번으로 이동한다.
...
- 검색 실패, 검색 결과 없음, 최종 답변 실패, TTS 보류 경로에서도 pending 문구가 `검색중...`에 고정되지 않게 한다.
...
- 실제 검색 API 호출 중에는 로딩 문구가 `검색중...`으로 바뀌고, 검색 후 최종 답변 생성 단계에서는 `생각중...`으로 돌아오는 테스트
- 검색 실패 시에도 pending 단계가 복구되는 테스트

## 성공 기준

- 사용자가 검색 기능을 켜고 Tavily API 키를 넣으면 `/search` 명령으로 최신 웹 검색 답변을 받을 수 있다.
- 자동 검색이 켜져 있으면 현재성이 중요한 질문에서 검색 결과가 답변에 반영된다.
- 검색 API 호출 중에는 채팅창이 `검색중...`을 표시하고, 답변 생성 단계에서는 다시 `생각중...`을 표시한다.
