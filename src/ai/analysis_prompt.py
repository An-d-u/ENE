...
_STRUCTURED_ANALYSIS_RULES_BY_LANGUAGE = {
    "ko": {
        "analysis": [
            ...
            "- 현재 기분의 표현 질감은 말투, 답변 길이, 제안 정도, 장난기와 다정함에 안전하게 반영하되, 허용된 stance 행동은 아래 기분 사건 규칙에 따라 수행하세요.",
    ...
    "en": {
        "analysis": [
            ...
            "- Reflect mood's expressive texture safely in tone, reply length, initiative, playfulness, and warmth; carry out permitted stance behavior under the mood event rules below.",
...
_STRUCTURED_ANALYSIS_RULES_BY_LANGUAGE["ja"] = {
    "analysis": [
        ...
        "- 気分の表現上の質感は、口調、返答の長さ、提案の度合い、遊び心、温かさに安全に反映し、許可されたstance行動は下の気分イベント規則に従ってください。",
...
_MOOD_SEMANTIC_RULES_BY_LANGUAGE = {
    "ko": [
        ...
        "- `repair_signal`이 `none`이 아니면 반드시 `kind=repair`, `target_scope=relationship`, 구체적인 `relation_category`를 함께 사용하세요.",
        "- `acknowledgment`는 사건을 인정하지만 해결을 주장하지 않는 신호이고, `apology`는 책임을 인정하는 사과이며, `explanation`은 명시적이고 확신도 높은 책임 회피가 아닌 맥락 설명으로 단독 해결 신호가 아닙니다.",
        "- `correction`은 같은 관계 category의 실제 교정, `follow_through`는 같은 관계 category의 실제 이행 증거에만 사용하세요.",
    ...
    "en": [
        ...
        "- A non-`none` `repair_signal` requires `kind=repair`, `target_scope=relationship`, and a concrete `relation_category`.",
        "- `acknowledgment` recognizes the event without claiming resolution, `apology` accepts responsibility, and `explanation` gives explicit, high-certainty context without evading responsibility and never resolves the rupture alone.",
        "- Use `correction` only for actual correction in the same relationship category and `follow_through` only for actual follow-through evidence in that same category.",
    ...
    "ja": [
        ...
        "- `repair_signal` が `none` 以外なら、必ず `kind=repair`、`target_scope=relationship`、具体的な `relation_category` を併用してください。",
        "- `acknowledgment` は出来事を認めても解決済みとはせず、`apology` は責任を認める謝罪、`explanation` は明示的で確信度が高い責任逃れではない文脈説明であり、単独では亀裂を解決しません。",
        "- `correction` は同じ関係categoryの実際の訂正、`follow_through` は同じ関係categoryの実際の履行証拠にだけ使ってください。",
