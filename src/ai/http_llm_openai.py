...
class OpenAICompatibleClient(_CommonMixin):
    ...
    async def send_message_with_images(
        ...
        self._remember_turn(parts, self._assistant_history_content_for_response(raw_response_text, (clean_text, emotion, tts_text, events, analysis, promises, thought, goal_update, proactive_conversations)))
...
class OpenAIResponseAPIClient(_CommonMixin):
    ...
    def send_message(self, message: str) -> LLM_RESPONSE_TUPLE:
        ...
        self._history.append({"role": "assistant", "content": self._assistant_history_content_for_response(raw_response_text, (clean_text, emotion, tts_text, events, analysis, promises, thought, goal_update, proactive_conversations))})
        return clean_text, emotion, tts_text, events, analysis, promises, thought, goal_update, proactive_conversations
