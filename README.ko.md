...
생활 기록은 앱이 실행되지 않았던 동안 ENE가 텍스트 기반 생활 환경에서 무엇을 했는지 기록합니다. 기본값은 비활성화입니다. `설정 → 행동 → 생활 기록`에서 켤 수 있으며, 최소 비활성 시간의 기본값은 60분이고 같은 곳에서 변경할 수 있습니다.

자유형 Markdown 생활 환경은 `설정 → 프롬프트 → 생활 환경`에서 편집합니다. 런타임 파일은 `%AppData%/ENE/prompts/life_world.md`입니다. 생활 환경을 의도적으로 비워 두면 생성을 건너뛰며 일반 채팅은 계속됩니다.

기준을 충족한 비활성 구간 뒤 첫 일반 채팅에서는 LLM을 순차적으로 두 번 호출할 수 있습니다. 먼저 생활 기록을 만들고, 가장 최근의 성공 기록을 임시 맥락에 포함해 일반 답변을 만듭니다. 해당 턴에 표시되는 토큰 사용량은 두 호출을 합산합니다. 명령은 첫 채팅 조건을 소비하지 않으며, 답변을 리롤해도 생활 기록을 다시 만들지 않습니다.

`··· → 생활 기록`에서 날짜별 기록을 탐색할 수 있습니다. 과거 기록은 읽기 전용이고 가장 최신 기록만 재생성할 수 있습니다. 재생성에 실패하면 기존 기록을 유지합니다. 기록 파일 손상이나 생성 실패가 발생해도 일반 채팅 답변은 계속됩니다.

ENE 데이터를 옮기거나 복원하기 전에는 다음 사용자 파일 3개를 함께 백업하세요.

- `%AppData%/ENE/life_records.json`
- `%AppData%/ENE/life_session_state.json`
- `%AppData%/ENE/prompts/life_world.md`

저장소, 복구, 개인정보 보호, 릴리스 점검 방법은 [생활 기록 운영 및 복구](docs/life_records.md)를 참고하세요.
...
일반 채팅은 기본 프롬프트, 코드에서 생성되는 분석/일정/약속 규칙, 생성된 런타임 계약, 보조 프롬프트 본문, 모델 인식 표정 지침을 조합합니다. `/note` 계획 흐름은 더 제한된 맥락을 사용하므로 일반 보조 프롬프트 경로와 다르게 동작합니다.
